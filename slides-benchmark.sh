...
sbt "Jmh/run -i $IT -wi $WIT -f1 -t1 .*chapter3.SetBenchmark_.*Invoice_HalfHit_Large.*" | tee -a chapter3-sets.report.txt
sbt "Jmh/run -i $IT -wi $WIT -f1 -t1 .*chapter3.MapBenchmark_Invoice_HalfHit_(Small|Large).*" | tee -a chapter3-maps.report.txt
sbt "Jmh/run -i $IT -wi $WIT -f1 -t1 .*chapter4.StatsBenchmark_(Small|Large).foreach.*" | tee -a chapter4-stats.report.txt
