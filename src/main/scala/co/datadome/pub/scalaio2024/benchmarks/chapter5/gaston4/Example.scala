...
object Example {
  ...
  val startingSchedule: Schedule = Schedule.from(
    Topics.DnD -> Seq("Arthur", "Georges", "Haley", "Isidore", "Jane").map(Persons.person),
    Topics.Cthulhu -> Seq("Bianca", "Kevin", "Lily", "Maurice").map(Persons.person),
    Topics.Vampire -> Seq("Charles", "Nicky", "Oliver", "Patricia", "Quentin").map(Persons.person),
    Topics.L5R -> Seq("Donna", "Eric", "Rita", "Steven", "Thomas").map(Persons.person),
    Topics.InsMv -> Seq("Fiona", "Ursula", "Veronica").map(Persons.person)
  )
}
