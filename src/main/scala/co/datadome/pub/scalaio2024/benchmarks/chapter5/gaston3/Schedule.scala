...
final case class Schedule(private val wrapped: ArrayMap[Topic, Record])(using problem: Problem) {

  private inline def updateWrapped(inline w: ArrayMap[Topic, Record]): Schedule =
    copy(wrapped = w)
  ...
  def swapPersons(tp1: (Topic, Person), tp2: (Topic, Person)): Schedule = updateWrapped {
    ...
    wrapped.updated(t1 -> newR1, t2 -> newR2)
  }
  ...
  def movePerson(source: Topic, destination: Topic, person: Person): Schedule = updateWrapped {
    ...
    wrapped.updated(source -> newSourceRecord, destination -> newDestinationRecord)
  }

  private lazy val scoresByPerson: ArrayMap[Person, Score] = {
    val result = ArrayMap.fill[Person, Score](problem.personsCount, Score.Zero)
    // Each person appears only once
    records.fastForeach { rec =>
      rec.persons.fastForeach { p =>
        result(p) = rec.scoresByPerson(p)
      }
    }
    result
  }
  ...
  lazy val totalScore: Score =
    scoresByPerson.values.sorted(Score.ReverseOrdering).fastFoldLeft(Score.Zero) { case (acc, s) => (acc * Schedule.RankFactor) + s }

  override def toString: String = {
    val content = wrapped.values.map(_.toString).sorted.mkString("\n")
    val scores = problem.personsSet.map { person =>
      val score = scoresByPerson(person)
      s"${person.name}: $score"
...
object Schedule {
  ...
  def from(entries: Record*)(using Problem): Schedule = {
    val arraymap: ArrayMap[Topic, Record] = entries.map(e => e.topic -> e).toMap.toArrayMap
    new Schedule(arraymap)
  }
}
