...
final case class Schedule(private val wrapped: Map[Topic, Record])(using problem: Problem) {
  ...
  private lazy val scoresByPerson: Map[Person, Score] = records.fastFoldLeft(Map.empty)(_ ++ _.scoresByPerson)

  lazy val totalScore: Score =
    scoresByPerson.values.toSeq.sorted(Score.ReverseOrdering).fastFoldLeft(Score.Zero) { case (acc, s) => (acc * Schedule.RankFactor) + s }
