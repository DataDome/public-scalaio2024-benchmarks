...
object Example {
  ...
  private object Topics {
    ...
    val DnD: Topic = Topic(0, "Dungeons & Dragon", mandatory = Set(person("Arthur")).toArraySet(Persons.Count), min = 4, max = 7)
    val Cthulhu: Topic = Topic(1, "Call of Cthulhu", mandatory = Set(person("Bianca"), person("Lily")).toArraySet(Persons.Count), min = 3, max = 5)
    val Vampire: Topic = Topic(2, "Vampire", mandatory = Set(person("Charles")).toArraySet(Persons.Count), min = 3, max = 6)
    val L5R: Topic = Topic(3, "Legend of the Five Rings", mandatory = Set(person("Donna"), person("Eric")).toArraySet(Persons.Count), min = 4, max = 6)
    val InsMv: Topic = Topic(4, "INS/MV", mandatory = Set(person("Fiona")).toArraySet(Persons.Count), min = 3, max = 6)
